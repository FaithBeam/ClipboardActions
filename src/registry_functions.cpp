...
void set_startup() {
    ...
    HKEY k{};
    LSTATUS open_result = RegOpenKeyExW(HKEY_CURRENT_USER, L"SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Run\\", 0,
                                        KEY_ALL_ACCESS, &k);
    LSTATUS result = RegSetKeyValueW(k, nullptr, L"ClipboardActions", REG_SZ, path.c_str(),
...
void unset_startup() {
    HKEY k;
    LSTATUS result = RegOpenKeyExW(HKEY_CURRENT_USER, L"SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Run\\", 0,
                                   KEY_ALL_ACCESS, &k);
